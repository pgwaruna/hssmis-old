...
//echo$notid.$subbtn.$subvl;
	...
	if($subbtn=="Remove"){
		...
		echo"Are you sure,you want remove this notice? $confdel";
...
{	
	...
	//include 'Excel_Upload/Regi_Detail_UP.php';
		...
		$queinsfile="insert into notice(Title,File_Name,date_time,Status) values ('$upfltitle','$filename','$dnt',1)";
	...
	echo "<font size=3px> The file <font color=blue size=3px>".$filename."</font> has been uploaded<br></font> ";
} 
else
{
...
fclose($fh);
	...
	$queinsfl="insert into notice(Title,File_Name,date_time,Status) values ('$nttitle','$filename','$dnt',1)";
	//echo$queinsfl;
	mysql_query($queinsfl);
	echo"<font color=red>Notice Send Successfully ! </font>";
}
/////////////////////////////end create note ////////////////////////////////
...
$quedispnttbl="select * from notice order by date_time DESC";
$qudispnttbl=mysql_query($quedispnttbl);
if(mysql_num_rows($qudispnttbl)!=0){

echo"<table border=0 ><tr><th>No<th>Notice Title<th>File Name<th>Date and Time<th>Current Status<th colspan=2>Submit as</tr>";
$i=1;
...
echo"<form method=POST action='./index.php?view=admin&admin=53&task=modntst'><tr bgcolor='#edd4dc'><td align='center'>$i<input type=hidden name=notid value='$Notice_ID'><td>$Title<td>$File_Name<td align='center'>$date_time<td align='center'>$Status3<td align='center'>";
	if($Status2=='Hide'){
...
echo"Sorry! There are no Notices to Find.";
