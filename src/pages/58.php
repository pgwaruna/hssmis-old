...
$queprtatt="select code, name,stream,medium from courseunit where availability=1 and (semister=$crrseme or semister=3) order by code,name";
                                                                    }
elseif(($role=="general")||($role=="office")){
$queprtatt="select code, name,stream,medium from courseunit where department='$dept_id'  and availability=1 and (semister=$crrseme or semister=3)  order by code,name";
}
else{
$queprtatt="select code, name,stream,medium from courseunit where (coordinator='$rltduser' or lecturers LIKE '%[$rltduser]%')   and availability=1 and (semister=$find_L or semister=3)  order by code,name";	
}
