...
//echo$filcode.$lect_name.$peer_name.$sub_date;
/////////////////////////////////////////////////////////////////////////////
/////////////////////////// edit form////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
if($redt=="updt"){
...
//echo$uplect_name.$uprwacyear.$uppeer_name.$upsub_date.$edtpid.$submopt;
///////////////////////////////////////////////////////////////////////
///////////////////////////Update Form/////////////////////////////////
///////////////////////////////////////////////////////////////////////
if($submopt=="Update Form"){
$queupdtpidses="update peer_evaluation_session set ac_year='$uprwacyear',lect_name='$uplect_name',Peer_name='$uppeer_name',submit_date='$upsub_date' where peer_evl_id=$edtpid";
...
if($quupdtpidses){
	for($uq=1;$uq<=$nofqn;$uq++){
		...
		//echo$getneans;
			$queupdtans="update  peer_evaluation_status set answer='$getneans' where que_no=$uq and peer_evl_id=$edtpid";
...
if($submopt=="Delete Form"){
	$quedelses="delete from  peer_evaluation_session where peer_evl_id=$edtpid";
	$qudelses=mysql_query($quedelses);

		if($qudelses){
			$quedelsts="delete from peer_evaluation_status where peer_evl_id=$edtpid";
...
//////////////////////////////////////////////////////////////////////////////
	...
	$queinsprses="insert into  peer_evaluation_session(course_unit,ac_year,lect_name,Peer_name,submit_date) values('$filcode', '$filacyear', '$lect_name', '$peer_name','$sub_date')";
	$quinsprses=mysql_query($queinsprses);

	if($quinsprses){
		$quegetmxpid="SELECT MAX(peer_evl_id) as maxpid FROM peer_evaluation_session";
		...
		for($grd=1;$grd<=$nofque;$grd++)
			...
			$rdbttn=$_POST[$grd];
			//echo$grd."-".$rdbttn."<br>";
			$queinsqnqns="insert into peer_evaluation_status(peer_evl_id,que_no,answer) values ($getmaxp_id,$grd,'$rdbttn')";
	...
	echo"<font color=blue>Form Succsefuly Inserted.<br>(If you want to edit previous session Please use following table unless ignore this table.)</font>";
		...
		$quegetprvdata="select * from peer_evaluation_session where peer_evl_id=$getmaxp_id";
		$qugetprvdata=mysql_query($quegetprvdata);
		echo"<table border=0 width=100%><tr class=faccolor><td align=center><font color=#ffffff><b>Lecturer Name &<br> Review Academic Year<td align=center><b><font color=#ffffff> Name of the Peer &<br> Submit Date";
			for($q=1;$q<=$nofque;$q++){
		...
		while($qgetprvdata=mysql_fetch_array($qugetprvdata)){
				...
				$Peer_name=$qgetprvdata['Peer_name'];
				$submit_date=$qgetprvdata['submit_date'];
		echo"<form method=post action='./index.php?view=admin&admin=70&task=setevlfm&due=entbl&redt=updt' id='cnfdelfm'><tr class=trbgc>";
			...
			echo"<td align=center><input type=text name=uppeer_name value='$Peer_name' size=15><br>";
			echo"<input type=text name=upsub_date value='$submit_date' size=15>";


				$quegetprvans="select * from peer_evaluation_status where peer_evl_id=$getmaxp_id order by que_no";
				...
				while($qgetprvans=mysql_fetch_array($qugetprvans)){				
						...
						echo"<td align=center>";
							echo"<select name=recrtans$gtquen  width='46' style='width: 46px'>";
...
//////////////////////////////////////////////////////////
	...
	//echo$evlcd;
$quegetque="select * from  peer_evaluation_questions where que_version=1 order by que_no";
$qugetque=mysql_query($quegetque);
if(mysql_num_rows($qugetque)==0){
	echo"<font color=red>Sorry! Can not find Questions.</font>";
					}
else{
echo"<div align=right>[ <a href='./index.php?view=admin&admin=70'>Back to Course Unit List </a>]</div>";
echo"<form method=post action='./index.php?view=admin&admin=70&task=setevlfm&due=entbl'>";
echo"<table border=0><th colspan=8 align center height=30px>Peer Evaluation Form Submit for $evlcddis";
echo"<input type=hidden name=filcode value='$evlcd'>";

//////////////////////////////////lect name//////////////////////////////////////////////////////////////////
echo"<tr class=trbgc height=30px><td colspan=2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Name of the Lecturer: ";
...
//////////////////////////////////peer name adn date///////////////////////////////////////
echo"<tr class=trbgc height=30px><td colspan=2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Name of the Peer: ";
	echo"<input type=text name=peer_name size=30>";
echo"<td colspan=6>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Evaluated Date : ";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
echo"<br><font size=3px>*** Select Course Unit to Enter Peer Evaluation Forms *** </font><br>";
//$role="lecturer";
...
echo"<table border=0><tr height=30px><th>#<th>Course Unit<th>Course Name<th>Department<th>Submit</tr>";
$rn=1;
while($qgetcodelst=mysql_fetch_array($qugetcodelst)){
		...
		$percddept=$qgetcodelst['department'];
			...
			///////////////////////////////////////////////////////////////
			///////////////////////////////////////////////////////////////
	echo"<form method=post action='./index.php?view=admin&admin=70&task=setevlfm'>";
	...
	echo"<input type=submit value='Submit Forms for $fulcode'>";
