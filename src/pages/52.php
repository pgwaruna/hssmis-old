...
$queprtatt="select code, name,department,level,medium,stream from courseunit where availability=1 and (semister=$find_L or semister=3) and by_low_version=$curriculum order by code,name";
                                                                    }
elseif(($role=="general")||($role=="office")){
$queprtatt="select code, name,department,level,medium,stream from courseunit where department='$dept_id'  and availability=1 and (semister=$find_L or semister=3) and by_low_version=$curriculum order by code,name";
}
else{
$queprtatt="select code, name,department,level,medium,stream from courseunit where (coordinator='$rltduser' or lecturers LIKE '%[$rltduser]%')   and availability=1 and (semister=$find_L or semister=3) and by_low_version=$curriculum order by code,name";
}
...
while($qprtatt=mysql_fetch_array($quprtatt)){
	...
	$medium=$qprtatt['medium'];
		...
		echo"<tr class=trbgc><form method=POST action='./forms/form_52.php'><td align='center'>$pntatst<td align='center'>".$fulcode;
		...
		if($medium!="SI+EN"){
			echo"<input type='hidden' name='prtcodept' value= $codept><input type='hidden' name='prtcomdm' value=$medium><input type=submit value='View Attendance Sheet'></td></tr></form>";
		}
		else{
			echo"<input type='hidden' name='prtcomdm' value=$medium>";
				echo"<table border=0>";
				echo"<tr class=selectbg height=25px><td>";
				echo"<input type='hidden' name='prtcodept' value= $codept><input name=assubbtn type=submit value='View Attendance Sheet - Sinhala'></td></tr>";
				echo"<tr class=selectbg height=25px><td>";
				echo"<input type='hidden' name='prtcodept' value= $codept><input name=assubbtn  type=submit value='View Attendance Sheet - English'></td></tr></form>";
				echo"</table>";				
