...
echo"Adding Medicals for Attendace to One Student";
echo"<hr class=bar>";


echo"<table class=bgc><tr align=center><td><br>";
echo"<form method=POST action='./index.php?view=admin&admin=92&task=setex'>";
		...
		echo"( $stname - HS/$exstbt/$exstno )</font><br>";
			...
			echo"<table border=0 width=100%><tr><th>Course Unit</th><th>Course Name</th><th>Modification</th></tr>";
				...
				while($qexccos=mysql_fetch_array($quexccos)){
					...
					echo"<tr class=trbgc height=25px><td align='center'>".$fulcode."</td><td>&nbsp; &nbsp;$excorsnm</td><td align='center'>[ <a href='index.php?view=admin&admin=92&task=setex&due=chng&exco=$excors'>Add Medical for $fulcode</a> ]</td></tr>";
...
//////////////////////////////////////////////////////set as excuses/////////////////////////////////////////////////////////////
	if($subdue=="excues"){
		...
		if(mysql_num_rows($quchklecid)!=0){
			...
			$queinsexco="insert into participation(student,lect_id,status) values('hs$exstno',$excoid,2)";
			//echo$queinsexco;
...
//////////////////////////////////////////////////////remove excuses/////////////////////////////////////////////////////////////
	if($subdue=="excancel"){
		...
		if(mysql_num_rows($quchklecid)!=0){
			//echo$excoid.$exco.$exstno;
			$quedelexco="delete from participation where student='hs$exstno' and lect_id=$excoid and status=2";
		...
		if($pgrp2=="nogrp"){
			...
			if(($pgrp2=="nogrp")||($stgp=="$grop")||($stgp=="ng")){
				if(mysql_num_rows($qugetatt)==0){
					...
					echo"<td align='center'>[ <a href='index.php?view=admin&admin=92&task=setex&due=chng&subdue=excues&exco=$exco&excoid=$lectid'>Set as Medical</a> ]</tr>";
					...
					$qgetatt=mysql_fetch_array($qugetatt);
						...
						if($stat==2){
							...
							echo"<td align='center'><font color=red>(Set as Present)</font>&nbsp;&nbsp;&nbsp;&nbsp;[ <a href='index.php?view=admin&admin=92&task=setex&due=chng&subdue=excancel&exco=$exco&excoid=$lectid'>Cancel</a> ]</tr>";
